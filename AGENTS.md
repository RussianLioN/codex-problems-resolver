...
- `scripts/` — команды управления на Python 3 без внешних зависимостей; точные интерфейсы добавляются в рамках текущей начальной настройки.
...
Базовые команды репозитория должны использовать Python 3 и только стандартную библиотеку. В рамках текущей начальной настройки добавляются точные интерфейсы:
...
После появления этих файлов они являются обязательными командами проверки для запросов на слияние; указывайте фактически выполненные команды и их результат. Команда применения политики является изменяющей операцией и допускается только явно, с подтверждением:
